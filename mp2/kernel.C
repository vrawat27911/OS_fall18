...
int main() {
    ...
    // head of sequence error on Console
    // Console::puti(kernel_mem_pool.get_frames(13)); Console::puts("\n");
    // ContFramePool::release_frames(524);
    ...
    ContFramePool process_mem_pool(PROCESS_POOL_START_FRAME,
                                   PROCESS_POOL_SIZE, // (16 MB)/ (4 KB)
                                   process_mem_pool_info_frame,
                                   n_info_frames);
    
    process_mem_pool.mark_inaccessible(MEM_HOLE_START_FRAME, MEM_HOLE_SIZE);
    // mark all inaccessible test
    // process_mem_pool.mark_inaccessible(PROCESS_POOL_START_FRAME, PROCESS_POOL_SIZE);
    
    // multiple pools test    
    // unsigned long process_info_frame2 = kernel_mem_pool.get_frames(1);
    // ContFramePool process_mem_pool2 ((20 MB)/ (4 KB), (4 MB)/ (4 KB), 0, 0);
    ...
    Console::puts("\n Testing kernel frame pool. \n");
    test_memory(&kernel_mem_pool, 32);
    // multiple allocation n deallocation
    // test_memory(&kernel_mem_pool, 32);

    // test memory with process pool with diff sizes
    // Console::puts("\n Additonal Testing of process frame pool \n");
    // test_memory(&process_mem_pool2, 4);
    // test_memory(&process_mem_pool, 16);
